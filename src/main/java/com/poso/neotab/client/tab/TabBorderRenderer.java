...
public final class TabBorderRenderer {
    ...
    public static void drawRainbowBorder(GuiGraphics guiGraphics, int left, int top, int right, int bottom) {
        ...
        // 顶点写入顺序与 1.21.1 fill 反编译结果完全一致，因此绘制顺序和视觉效果不变。
    ...
    /**
     * 向顶点缓冲写入一个 1×1 像素四边形。顶点顺序必须与 1.21.1 GuiGraphics.fill 反编译结果一致
     * （(x,y)→(x,y+1)→(x+1,y+1)→(x+1,y)，z=0，颜色经 setColor(int-ARGB) 写入）：
     * gui 渲染类型开启背面剔除，绕向写反会被整面剔除。
     */
    private static void fillPixelQuad(VertexConsumer buffer, Matrix4f pose, int x, int y, int color) {
        ...
        buffer.addVertex(pose, x,     y + 1, 0.0F).setColor(color);
        buffer.addVertex(pose, x + 1, y + 1, 0.0F).setColor(color);
        buffer.addVertex(pose, x + 1, y,     0.0F).setColor(color);
    }
