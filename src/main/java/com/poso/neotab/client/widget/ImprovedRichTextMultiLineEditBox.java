 ...
 * </ul>
 * 
 * <p>注意：语法高亮功能暂时禁用，以避免生产环境中的反射问题。</p>
 */
public class ImprovedRichTextMultiLineEditBox extends NoCountMultiLineEditBox {
     ...
     * 
     * <p>注意：由于生产环境中的 Mixin Accessor 问题，暂时禁用语法高亮功能。</p>
     */
    @Override
    protected void renderContents(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        // 临时禁用语法高亮，使用父类的默认渲染
        // TODO: 使用 Access Transformer 或其他方案来访问私有字段
        super.renderContents(guiGraphics, mouseX, mouseY, partialTick);
    }

    // 以下是原来的语法高亮实现代码，暂时注释掉
    /*
    @Override
    protected void renderContents(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        // 直接调用 Accessor 方法（this 实现了 MultiLineEditBoxAccessor 接口）
        MultilineTextField textField = this.neotab$getTextField();
        Font font = this.neotab$getFont();
        Component placeholder = this.neotab$getPlaceholder();
        ...
        if (textField.hasSelection()) {
            MultilineTextFieldAccessor textFieldAccessor = (MultilineTextFieldAccessor) (Object) textField;
            int selBegin = Math.min(textFieldAccessor.neotab$getCursor(), textFieldAccessor.neotab$getSelectCursor());
            int selEnd   = Math.max(textFieldAccessor.neotab$getCursor(), textFieldAccessor.neotab$getSelectCursor());
