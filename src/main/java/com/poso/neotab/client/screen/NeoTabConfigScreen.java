...
public class NeoTabConfigScreen extends Screen {
    ...
    public <T extends AbstractWidget> T addWidgetHelper(T widget) { 
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollDelta) {
        ...
        if (hovered instanceof ImprovedRichTextMultiLineEditBox input) {
            ...
            if (onInputScrollbar) return input.mouseScrolled(mouseX, mouseY, scrollDelta);
        }
        if (!isInsideViewport(mouseX, mouseY, layout) || layout.maxScroll() <= 0) return false;
        // 滚动时隐藏颜色选择器（对所有颜色类型）
    ...
    private void save() {
        if (screenMode == ScreenMode.PLAYER) {
            ...
            com.poso.neotab.network.NeoTabNetwork.INSTANCE.send(PacketDistributor.SERVER.noArg(), new SavePlayerConfigPacket(playerCfg));
        } else {
